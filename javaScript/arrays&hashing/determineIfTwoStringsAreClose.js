...
const closeStrings = (word1, word2) => {
  ...
  for (let char of word1) mapWord1.set(char, (mapWord1.get(char) | 0) + 1);
  for (let char of word2) mapWord2.set(char, (mapWord2.get(char) | 0) + 1);

  const keys1 = [...mapWord1.keys()].sort();
  const keys2 = [...mapWord2.keys()].sort();

  if (keys1.join('') !== keys2.join('')) return false;

  const values1 = [...mapWord1.values()].sort((a, b) => a - b);
  const values2 = [...mapWord2.values()].sort((a, b) => a - b);
